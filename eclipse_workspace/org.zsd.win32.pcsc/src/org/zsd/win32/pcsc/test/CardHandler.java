...
public class CardHandler implements Runnable {
	...
	public void run() {
		...
		try {
			...
			bytes = PCSCAPI.SCardGetAttrib(hContext, hCard, PCSCAPI.SCARD_ATTR_VENDOR_IFD_VERSION);
			if(bytes != null) {
				logger.debug("SCARD_ATTR_VENDOR_IFD_VERSION = " + new String(bytes));
			}
			bytes = PCSCAPI.SCardGetAttrib(hContext, hCard, PCSCAPI.SCARD_ATTR_VENDOR_IFD_SERIAL_NO);
			if(bytes != null) {
				logger.debug("SCARD_ATTR_VENDOR_IFD_SERIAL_NO = " + new String(bytes));
			}
			bytes = PCSCAPI.SCardGetAttrib(hContext, hCard, PCSCAPI.SCARD_ATTR_ATR_STRING);
			if(bytes != null) {
				logger.debug("SCARD_ATTR_ATR_STRING = " + new String(bytes));
			}
			bytes = PCSCAPI.SCardGetAttrib(hContext, hCard, PCSCAPI.SCARD_ATTR_DEVICE_FRIENDLY_NAME);
			if(bytes != null) {
				logger.debug("SCARD_ATTR_DEVICE_FRIENDLY_NAME = " + new String(bytes));
			}
			bytes = PCSCAPI.SCardGetAttrib(hContext, hCard, PCSCAPI.SCARD_ATTR_DEVICE_SYSTEM_NAME);
			if(bytes != null) {
				logger.debug("SCARD_ATTR_DEVICE_SYSTEM_NAME = " + new String(bytes));
			}
